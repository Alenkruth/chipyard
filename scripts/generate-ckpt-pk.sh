#!/bin/bash
# this script uses spike pk (proxy kernel) by default. If you do not want to use the proxy kernel, you should use the non-pk script to make your checkpoints.
...
echo "spike -d --debug-cmd=$CMDS_FILE $SPIKEFLAGS pk $BINARY" > $SPIKECMD_FILE

echo "Capturing state at checkpoint to spikeout"
echo $NHARTS > $LOADARCH_FILE
spike -d --debug-cmd=$CMDS_FILE $SPIKEFLAGS pk $BINARY 2>> $LOADARCH_FILE
